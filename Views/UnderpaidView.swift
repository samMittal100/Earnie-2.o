...
struct UnderpaidView: View {
    ...
    var body: some View {
        ZStack {
            if shouldRedirect {
                // Redirects to your Analysis page after 5 seconds
        ...
        .onAppear {
            // MARK: - Redirect Timer
            // Waits for 5 seconds and then switches the view
    ...
    // MARK: - Liquid Glass Morphism Tab Bar
    var glassTabBar: some View {
        ...
        .background(
            ZStack {
                ...
                RoundedRectangle(cornerRadius: 28, style: .continuous)
                    .stroke(LinearGradient(colors: [.white.opacity(0.5), .white.opacity(0.2)], startPoint: .topLeading, endPoint: .bottomTrailing), lineWidth: 0.5)
            }
        )
